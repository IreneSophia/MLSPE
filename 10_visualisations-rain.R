...
pit_var = ggplot(df.indi, aes(x = task, y = pit_var, fill = `diagnostic status`)) +
  geom_flat_violin(aes(fill = `diagnostic status`),position = position_nudge(x = .2, y = 0), 
                   adjust = 1.5, trim = TRUE, alpha = .75, colour = NA)+
  geom_point(aes(x = as.numeric(task)-.25, y = pit_var, colour = `diagnostic status`),
             position = position_jitter(width = 0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = pit_var, fill = `diagnostic status`), 
               outlier.shape = NA, alpha = .75, width = wb, colour = "black")+
  ...
  theme(plot.title = element_text(hjust = 0.5, size = st), # text = element_text(size=sz), 
...
int_var = ggplot(df.indi, aes(x = task, y = int_var, fill = `diagnostic status`)) +
  geom_flat_violin(aes(fill = `diagnostic status`),position = position_nudge(x = .2, y = 0), 
                   adjust = 1.5, trim = TRUE, alpha = .75, colour = NA)+
  geom_point(aes(x = as.numeric(task)-.25, y = int_var, colour = `diagnostic status`),
             position = position_jitter(width=0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = int_var, fill = `diagnostic status`),
               outlier.shape = NA, alpha = .75, width = wb,colour = "black")+
  ...
  theme_bw()+ 
  theme(text = element_text(size=sz), plot.title = element_text(hjust = 0.5, size = st),
        legend.position = "none")#, axis.title.y = element_blank())
...
art = ggplot(df.indi, aes(x = task, y = art, fill = `diagnostic status`)) +
  ...
  geom_point(aes(x = as.numeric(task)-.25, y = art, colour = `diagnostic status`),position = position_jitter(width=0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = art, fill = `diagnostic status`),outlier.shape = NA, alpha = .75, width = wb,colour = "black")+
...
pit_sync = ggplot(df.indi, aes(x = task, y = pit_sync, fill = `diagnostic status`)) +
  geom_flat_violin(aes(fill = `diagnostic status`),position = position_nudge(x = .2, y = 0), adjust = 1.5, trim = TRUE, alpha = .75, colour = NA)+
  geom_point(aes(x = as.numeric(task)-.25, y = pit_sync, colour = `diagnostic status`),position = position_jitter(width=0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = pit_sync, fill = `diagnostic status`),outlier.shape = NA, alpha = .75, width = wb,colour = "black")+
...
int_sync = ggplot(df.indi, aes(x = task, y = int_sync, fill = `diagnostic status`)) +
  geom_flat_violin(aes(fill = `diagnostic status`),position = position_nudge(x = .2, y = 0), adjust = 1.5, trim = TRUE, alpha = .75, colour = NA)+
  geom_point(aes(x = as.numeric(task)-.25, y = int_sync, colour = `diagnostic status`),position = position_jitter(width=0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = int_sync, fill = `diagnostic status`),outlier.shape = NA, alpha = .75, width = wb,colour = "black")+
  scale_y_continuous(name = "correlation coefficient") +
  labs(x = '') +
  scale_colour_manual(values = c("autistic" = gr, "non-autistic" = bl)) +
  scale_fill_manual(values = c("autistic" = gr, "non-autistic" = bl)) + 
...
p = ggarrange(pit_var, int_var, art, pit_sync, int_sync, art_sync, legend,
          ...
          font.label = list(size = sz, vjust = 20),
          ncol = 3, nrow = 3)

ggsave("indi.png", width = 180, height = 144, units = "mm")
...
str = ggplot(df.dyad, aes(x = task, y = str, fill = `dyad type`)) +
  ...
  geom_point(aes(x = as.numeric(task)-.25, y = str, colour = `dyad type`),position = position_jitter(width=0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = str, fill = `dyad type`),outlier.shape = NA, alpha = .75, width = wb,colour = "black")+
...
ttg = ggplot(df.dyad, aes(x = task, y = ttg, fill = `dyad type`)) +
  geom_flat_violin(aes(fill = `dyad type`),position = position_nudge(x = .2, y = 0), adjust = 1.5, trim = TRUE, alpha = .75, colour = NA)+
  geom_point(aes(x = as.numeric(task)-.25, y = ttg, colour = `dyad type`),position = position_jitter(width=0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = ttg, fill = `dyad type`),outlier.shape = NA, alpha = .75, width = wb,colour = "black")+
...
pit_sync_MEA = ggplot(df.dyad, aes(x = task, y = pit_sync_MEA, fill = `dyad type`)) +
  geom_flat_violin(aes(fill = `dyad type`),position = position_nudge(x = .2, y = 0), adjust = 1.5, trim = TRUE, alpha = .75, colour = NA)+
  geom_point(aes(x = as.numeric(task)-.25, y = pit_sync_MEA, colour = `dyad type`),position = position_jitter(width=0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = pit_sync_MEA, fill = `dyad type`),outlier.shape = NA, alpha = .75, width = wb,colour = "black")+
  scale_y_continuous(name = "correlation coefficient") +
  labs(x = '') +
  scale_colour_manual(values = c("heterogeneous" = gr, "homogeneous" = bl)) +
  scale_fill_manual(values = c("heterogeneous" = gr, "homogeneous" = bl)) + 
...
int_sync_MEA = ggplot(df.dyad, aes(x = task, y = int_sync_MEA, fill = `dyad type`)) +
  geom_flat_violin(aes(fill = `dyad type`),position = position_nudge(x = .2, y = 0), 
                   adjust = 1.5, trim = TRUE, alpha = .75, colour = NA)+
  geom_point(aes(x = as.numeric(task)-.25, y = int_sync_MEA, colour = `dyad type`),
             position = position_jitter(width=0.075), size = sp, shape = 20)+
  geom_boxplot(aes(x = task, y = int_sync_MEA, fill = `dyad type`),
               outlier.shape = NA, alpha = .75, width = wb,colour = "black")+
  scale_y_continuous(name = "correlation coefficient") +
  labs(x = '') +
  scale_colour_manual(values = c("heterogeneous" = gr, "homogeneous" = bl)) +
...
ggsave("dyad.png", width = 180, height = 96, units = "mm")
